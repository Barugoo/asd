...
void vector_write(vector *v, const char* filename)
{
	FILE *fp; 
	...
	size_t length;
	for (i = 0; i < v->len; i++) {
		length = v->items[i]->length;
		fwrite(&length, sizeof(length), 1, fp);
		fwrite(v->items[i]->value, 1, length, fp);
	}
	...
	while (input != 0){
		...
		switch(input){
			...
			case 6:
			{
				...
				printf("input filename (string): ");
				scanf("%s\n",filename);
				vector_write(&v, filename);
				break;
			}		
		}
	}
